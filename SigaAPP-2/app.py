...
import os, re, urllib.parse, pytz, json
...
# Filtro para URL encoding (usado para gerar link do WhatsApp)
...
# Rota de registro de solicitação
...
def register_solicitation():
    if request.method == 'POST':
        ...
        # Se a origem for "Ligação", "WhatsAPP" ou "Pessoalmente", valida o telefone
        if origin in ["Ligação", "WhatsAPP", "Pessoalmente"]:
            pattern = r'^\d{10,11}$'
            if not re.match(pattern, telefone):
                flash("Telefone inválido. Use o formato: +55XXXXXXXXXXX (10 ou 11 dígitos após +55).")
...
def dashboard():
    ...
    categories = ["RECLAMAÇÃO", "SUGESTÃO", "DÚVIDAS", "PEDIDOS", "DOENÇA"]
...
def reports_filter():
    ...
    solicitations = query.order_by(Solicitation.id.desc()).all()
    return render_template('reports_list.html', solicitations=solicitations, category=category)
